...
{
    volatile uint32_t lWaitCycles;

    if (delayUs == 0U) {
        return;
    }

    lWaitCycles = (SystemCoreClock / 5000000U) * delayUs;
    if (lWaitCycles == 0U) {
        lWaitCycles = delayUs;
    }

    while (lWaitCycles-- > 0U) {
        __NOP();
    }
}
...
{
    drvGpioWrite(DRVGPIO_TM1651_CLK,
                 releaseHigh ? DRVGPIO_PIN_SET : DRVGPIO_PIN_RESET);
}

static void tm1651PortDriveSda(bool releaseHigh)
{
    drvGpioWrite(DRVGPIO_TM1651_SDA,
                 releaseHigh ? DRVGPIO_PIN_SET : DRVGPIO_PIN_RESET);
}
...
static const uint8_t gTm1651PortBooSegData[3] = {
    0x7CU,
    0x3FU,
    0x3FU,
};
...
{
    if ((cfg == NULL) || !tm1651PlatformIsValidAssemble(device)) {
        return DRV_STATUS_INVALID_PARAM;
    }

    cfg->linkId = tm1651PlatformGetLinkId(device);
    return DRV_STATUS_OK;
}
...
eDrvStatus tm1651PortShowBoo(void)
{
    ...
    return tm1651DisplayRaw(TM1651_DEV0, gTm1651PortBooSegData, 3U);
}
