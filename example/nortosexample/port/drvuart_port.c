...
eDrvStatus drvUartGetPlatformStorageConfig(uint8_t uart, uint8_t **storage, uint32_t *capacity)
{
    ...
    switch ((eDrvUartPortMap)uart) {
        case DRVUART_UART1:
            *storage = gBspUartRxStorageUart1;
            *capacity = sizeof(gBspUartRxStorageUart1);
            return DRV_STATUS_OK;
        case DRVUART_WIFI:
            *storage = gBspUartRxStorageWifi;
            *capacity = sizeof(gBspUartRxStorageWifi);
            return DRV_STATUS_OK;
        default:
            *storage = NULL;
            *capacity = 0U;
