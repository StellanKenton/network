...
static OS_TICK mqtt_ms_to_ticks(CPU_INT32U time_ms)
{
	CPU_INT64U ticks;
	...
	ticks = ((CPU_INT64U)time_ms * (CPU_INT64U)OSCfg_TickRate_Hz + 999u) / 1000u;
	if (ticks == 0u) {
		ticks = 1u;
	}
...
static OS_TICK mqtt_now_tick(void)
{
	OS_TICK tick;
...
void mqtt_thread(void)
{
	...
	uint8_t t=0;
	OS_TICK keepalive_ticks;
	OS_TICK publish_ticks;
	OS_TICK curtick;
	OS_TICK sendtick;
	char *PASSWORD;
	...
	rc = OSTimeDlyHMSM(0,0,2,500);
	APP_RTOS_ASSERT((OS_ERR)rc);
	...
	curtick = mqtt_now_tick();
	sendtick = curtick;
	keepalive_ticks = (OS_TICK)(data.keepAliveInterval * OSCfg_TickRate_Hz);
	publish_ticks = (OS_TICK)(send_duration * OSCfg_TickRate_Hz);
	while(1)
	{
		if((mqtt_now_tick() - curtick) > keepalive_ticks)
		{
			if(msgtypes == 0)
			{
				curtick = mqtt_now_tick();
		...
		if(connect_flag == 1)
		{
			if((mqtt_now_tick() - sendtick) >= publish_ticks)
			{
				CPU_SR_ALLOC();
				sendtick = mqtt_now_tick();
				CPU_CRITICAL_ENTER();
...
//										LCD_ShowString(30,190,210,16,16,json_display->valuestring); 
		...
		OSTimeDly(mqtt_ms_to_ticks(100u));
	}
}
...
*/
void HexToStr(uint8_t *pbDest, const uint8_t *pbSrc, int nLen)
{
	char ddl,ddh;
	int i;
...
//	printf("\r\nlen = %d\r\n\r\n", len);
	...
	hmac_sha1((const uint8_t *)device_secret, strlen(device_secret), (const uint8_t *)content, strlen(content), buf, &len);
	HexToStr((uint8_t *)password, buf, (int)len);
}
