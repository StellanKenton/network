...
{  
  uint16_t ptr;
  uint8_t currentIf = 0xFFU;
  uint8_t epCount[USBH_MAX_NUM_INTERFACES];
  ...
  (void)memset(itf_desc, 0, sizeof(USBH_InterfaceDesc_TypeDef) * USBH_MAX_NUM_INTERFACES);
  (void)memset(ep_desc, 0, sizeof(USBH_EpDesc_TypeDef) * USBH_MAX_NUM_INTERFACES * USBH_MAX_NUM_ENDPOINTS);
  (void)memset(epCount, 0, sizeof(epCount));
  ...
  if (length > USB_CONFIGURATION_DESC_SIZE)
  {
    ...
    if ( cfg_desc->bNumInterfaces <= USBH_MAX_NUM_INTERFACES) 
    {
      ...
      {
        USBH_DescHeader_t *pdesc = (USBH_DescHeader_t *)(buf + ptr);

        if ((pdesc->bLength == 0U) || ((ptr + pdesc->bLength) > length))
        {
          break;
        }
        ...
        {
          uint8_t ifNumber = *(((uint8_t *)pdesc) + 2);
          uint8_t altSetting = *(((uint8_t *)pdesc) + 3);

          if ((ifNumber < USBH_MAX_NUM_INTERFACES) && (altSetting == 0U))
          {
            USBH_ParseInterfaceDesc(&itf_desc[ifNumber], (uint8_t *)pdesc);
            currentIf = ifNumber;
          }
          else
          {
        ...
        else if ((pdesc->bDescriptorType == USB_DESC_TYPE_ENDPOINT) && (currentIf < USBH_MAX_NUM_INTERFACES))
        {
          if (epCount[currentIf] < USBH_MAX_NUM_ENDPOINTS)
          {
            USBH_ParseEPDesc(&ep_desc[currentIf][epCount[currentIf]], (uint8_t *)pdesc);
            epCount[currentIf]++;
          }
        }
