...
static void USBH_LogEnumState(USBH_HOST *phost);
static void USBH_LogControlState(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, URB_STATE urbStatus);
...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  if ((HCD_IsDeviceConnected(pdev) == 0)&& (phost->gState != HOST_IDLE)) 
  {
    ...
    {
      LOG_W("usbCore", "disconnect hprt=0x%08x state=%u enum=%u ctrl=%u req=%u", (unsigned int)USB_OTG_READ_REG32(pdev->regs.HPRT0), (unsigned int)phost->gState, (unsigned int)phost->EnumState, (unsigned int)phost->Control.state, (unsigned int)phost->RequestState);
...
static USBH_Status USBH_HandleEnum(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)
{
  ...
  USBH_Status setCfgStatus;
  USBH_LogEnumState(phost);
  ...
  {
    uint16_t cfgDescLength = phost->device_prop.Cfg_Desc.wTotalLength;

    if (cfgDescLength > 512U) {
      cfgDescLength = 512U;
    }
  ...
  case ENUM_SET_CONFIGURATION:
    ...
    setCfgStatus = USBH_SetCfg(pdev,
                               phost,
                               phost->device_prop.Cfg_Desc.bConfigurationValue);
    if (setCfgStatus != USBH_BUSY) {
      LOG_W("usbCore", "set cfg status=%u val=%u hprt=0x%08x ctrl=%u req=%u", (unsigned int)setCfgStatus, (unsigned int)phost->device_prop.Cfg_Desc.bConfigurationValue, (unsigned int)USB_OTG_READ_REG32(pdev->regs.HPRT0), (unsigned int)phost->Control.state, (unsigned int)phost->RequestState);
    }
...
static void USBH_LogEnumState(USBH_HOST *phost)
{
  static ENUM_State lastEnumState = ENUM_IDLE;

  if (phost->EnumState != lastEnumState) {
    LOG_W("usbCore", "enum state=%u ctrl=%u req=%u", (unsigned int)phost->EnumState, (unsigned int)phost->Control.state, (unsigned int)phost->RequestState);
    lastEnumState = phost->EnumState;
  }
}
...
USBH_Status USBH_HandleControl (USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)
{
  ...
  {
  case CTRL_SETUP:
    USBH_LogControlState(pdev, phost, HCD_GetURB_State(pdev , phost->Control.hc_num_out));
    /* send a SETUP packet */
  ...
  case CTRL_SETUP_WAIT:
    ...
    USBH_LogControlState(pdev, phost, URB_Status);
  ...
  case CTRL_STATUS_IN:
    USBH_LogControlState(pdev, phost, HCD_GetURB_State(pdev , phost->Control.hc_num_in));
...
static void USBH_LogControlState(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, URB_STATE urbStatus)
{
  static CTRL_State lastControlState = CTRL_IDLE;
  static URB_STATE lastUrbStatus = URB_IDLE;
  uint8_t channel;

  if ((phost->Control.state != lastControlState) || (urbStatus != lastUrbStatus)) {
    channel = ((phost->Control.state == CTRL_STATUS_IN) || (phost->Control.state == CTRL_STATUS_IN_WAIT)) ? phost->Control.hc_num_in : phost->Control.hc_num_out;
    LOG_W("usbCore", "ctrl state=%u urb=%u hc=%u ch=%u hprt=0x%08x setup=%02x %02x %02x %02x %02x %02x %02x %02x",
          (unsigned int)phost->Control.state,
          (unsigned int)urbStatus,
          (unsigned int)HCD_GetHCState(pdev, channel),
          (unsigned int)channel,
          (unsigned int)USB_OTG_READ_REG32(pdev->regs.HPRT0),
          (unsigned int)phost->Control.setup.d8[0],
          (unsigned int)phost->Control.setup.d8[1],
          (unsigned int)phost->Control.setup.d8[2],
          (unsigned int)phost->Control.setup.d8[3],
          (unsigned int)phost->Control.setup.d8[4],
          (unsigned int)phost->Control.setup.d8[5],
          (unsigned int)phost->Control.setup.d8[6],
          (unsigned int)phost->Control.setup.d8[7]);
    lastControlState = phost->Control.state;
    lastUrbStatus = urbStatus;
  }
}
